import { FREE_MODELS, modelById, type FreeModel } from "@/lib/models";
import { getPuter, loadPuter, puterErrorMessage, type PuterChatMessage } from "@/lib/puter";

const CACHE_KEY = "horizon:last-free-model";
...
export type ChatFreeResult = {
  text: string;
  model: FreeModel;
};

export async function chatWithFreeModel(opts: {
  messages: PuterChatMessage[];
  pinnedModel?: string | null;
  onDelta: (text: string) => void;
}): Promise<ChatFreeResult> {
  await loadPuter();
  ...
  for (const testMode of [false, true]) {
    for (const model of models) {
      try {
        ...
        return { text, model: modelById(model.id) ?? model };
      } catch (err) {
        lastError = err;
      }
    }
